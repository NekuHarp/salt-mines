...
- Placing a bet requires an authenticated Salty Bet session (cookie), which is separate from the API's own Basic Auth. `src/shared/saltyBet.js` manages that session in memory:
    ...
    - `placeBet({ selectedplayer, wager, balance? })` — authenticates if there is no session, then POSTs the bet to `/ajax_place_bet.php`.
        - **The reply has two accepted shapes**, decided by `isAccepted`: a bare `"1"` (matchmaking and exhibition), or the balance with `1` appended (tournament) — `"15361"` at a balance of 1536. A **rejected** bet echoes the balance with no trailing flag (`"1536"`, e.g. betting after the window closed).
        - Both accepted shapes are honoured **regardless of match mode**, deliberately: `mode` can be deliberately overridden in the `Remaining` table, so it is not safe to key the parse on it.
        - The balance is therefore required to read the reply — for a balance ending in 1, accepted and rejected have the same shape (`"15311"` vs `"1531"`). `balance` is optional and read via `getBalance()` when omitted; pass it if you just read it. With no readable balance only the bare `"1"` can be recognised.
        - On rejection it reads the balance before retrying. Re-sending is harmless (Salty Bet keeps only the **latest** bet per match, so a resend replaces rather than adds) but wasteful, so it only retries when the balance is unreadable and an expired session is the plausible cause.
...
- The tier depends entirely on `mode`, which a `Remaining` row can deliberately override — see the match mode section. That override is the only mechanism for capping the wager on a phase the parser would call `Tournament`.
- `autoBet` passes the balance it just read into `placeBet`, both because the accept/reject reply can only be interpreted against it and to save a second read. A bet is recorded only when the reply is an accepted one; the balance cannot be used as a fallback check, since Salty Bet debits an accepted wager only at settlement (and at the tournament bailout floor a loss does not move it at all).
...
**The `Remaining` table is a deliberate override, not just a cache.** Because `resolveMatchMode` trusts a stored row over `getMatchMode`, a row can intentionally record a mode the parser would *not* derive, to change how that phase is treated. Mode is the only lever on wager sizing (`wagerPercent`), so this is how a phase gets a safer wager.

- **Do not "correct" rows that disagree with the parser** — check why they exist first. Current intentional entries:
    - `"17 characters are left in the bracket!"` → stored as **`Matchmaking`** (parser says `Tournament`). Salty Bet is bugged during this phase and bets it against the **regular balance instead of the tournament balance**, so letting it resolve to `Tournament` would go all-in with the real bankroll. Storing it as `Matchmaking` caps the wager at the 5–25% tiers.
    - `"FINAL ROUND! Stay tuned for exhibitions after the tournament!"` → `Tournament`, and `"Matchmaking mode will be activated after the next exhibition match!"` → `Exhibition`: strings the parser genuinely gets wrong, since neither contains the substring its phase is detected by.
- **Side effect of an override:** the `Prediction` row's `mode` comes from the same `resolveMatchMode` call, so an overridden match is *logged* under the substituted mode. The `"17 characters"` match therefore appears as `Matchmaking` despite running on tournament pots — detectable via `p1Total`/`p2Total` (tournament pots run ~1M vs ~5–10M for matchmaking), but not from `mode`. Worth filtering out of any training split.
