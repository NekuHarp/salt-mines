...
- Placing a bet requires an authenticated Salty Bet session (cookie), which is separate from the API's own Basic Auth. `src/shared/saltyBet.js` manages that session in memory:
    ...
    - `placeBet({ selectedplayer, wager })` — authenticates if there is no session, then POSTs the bet to `/ajax_place_bet.php`. The endpoint returns `1` on success / `0` on failure; `success` is `body === "1"`. **It does not blind-retry on failure.** A body other than `"1"` is not proof the bet was rejected, so re-sending risks a second live bet; it also cannot succeed for an all-in wager, whose funds are already committed. So on failure it reads the balance first: readable means the session is fine and the failure was something else (returns the failure as-is); unreadable means the session really expired and the bet cannot have landed, which is the only case that re-authenticates and retries.
...
- **A rejected-looking bet is confirmed against the balance before being discarded.** `autoBet` does not treat `success === false` as "no bet placed": it re-reads the balance, and if money left the account it records the bet anyway, using the observed decrease as the `wager` (the requested amount was never acknowledged). This exists because all-in tournament bets were landing while reporting failure, so every tournament bet was placed but logged with null bet columns.
